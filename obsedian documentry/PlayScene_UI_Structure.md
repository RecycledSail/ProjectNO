...
상단 상태 표시줄이다. `UIManager`가 `GameManager.Instance.dayUIEvent`에 연결되어 날짜 단위로 값을 갱신한다.
...
일반 팝업 패널 묶음이다. `UIManager.ReplacePopUp()`이 현재 팝업을 닫고 새 팝업을 열며, 내부적으로 이전 팝업을 스택에 저장한다.
...
- `InitBuildList()`가 `GlobalVariables.PRODUCTS.Keys`를 순회하며 `BuildItemPrefab`을 `BuildListParent`에 생성한다.
- `ManualBuildButton`은 `OnManualButtonClick()`으로 `BuildProvinceUI`를 연다.
- `UpdateQueue()`는 건설 예약 목록을 `BuildQueueItemPrefab`으로 표시한다.

### BuildProvinceUI

특정 건물을 지을 province를 선택하는 팝업이다.
...
- `UpdateBuildProvinceList()`가 현재 국가의 province 목록을 비교하며 필요한 항목만 추가/삭제한다.
...
- `InitProvinceList()`는 국가의 province를 `ProvinceButton` 프리팹으로 생성한다.
- `InitDiplomacyStats()`는 `AllyContent`, `EnemyContent` 아래에 국가 버튼을 동적으로 생성한다.
...
- `OpenFinanceUI()`에서 현재 국가를 설정하고 `InitFinanceView()`를 호출한다.
- 슬라이더 값은 0~100의 정수 퍼센트로 고정된다.
- 값 변경 시 `GovernmentBudget.Policy`에 금액으로 환산되어 반영된다.
...
- `OpenProvinceDetailUI(Province province)`에서 province 이름, 지형 설명, 시장/건물 리스트를 초기화한다.
- 시장 항목은 `ProductButtonUI`, 건물 항목은 `ProvinceBuildingButtonUI`로 생성된다.
...
- `OnSaveButtonClick()`은 입력 필드 값을 `GlobalVariables.saveFileName`에 넣고 저장 후 `MainMenuScene`으로 이동한다.

### FullUI

화면 전체를 쓰는 대형 UI 묶음이다. 현재 `ResearchUI`, `DiplomacyUI`는 비활성 상태로 배치되어 있다.
...
- `DiplomacyUI`는 씬에는 구성되어 있지만 대응 전용 스크립트는 아직 확인되지 않는다.
...
4. `GameManager.Instance.dayUIEvent`에 갱신 메서드를 연결한다.
...
5. 갱신 시 기존 자식을 제거하거나, `BuildProvinceUI`처럼 기존 항목을 재사용한다.
...
4. 일반 팝업과 동시에 열리지 않게 `UIManager`와 연동하거나 별도 전체 화면 UI 관리자를 둔다.
...
- `InitProvinceList()`, `InitRegimentList()`, `InitBuildList()`는 매번 모든 자식을 삭제하고 다시 만든다. 항목 수가 늘어나면 GC와 프레임 드랍이 생길 수 있다.
...
- `MarketUI.InitMarketList()`가 TODO 상태이므로 버튼은 열리지만 실제 내용이 비어 보일 수 있다.
...
- `GameManager.Instance.dayUIEvent` 구독은 대부분 `OnDestroy()`에서 해제하고 있어 좋다. 다만 `GameManager.Instance` 접근 전에 null 체크가 필요한 곳이 일부 있다.
